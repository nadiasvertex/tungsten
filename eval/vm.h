...
namespace tungsten {
    struct machine {
        ...
        template<typename T>
        std::size_t allocate() { return allocate(sizeof(T)); }
        ...
        template<typename T>
        void read_register(const vm::register_name &src, T &value) {
            memcpy(&value, &registers[src.index], sizeof(T));
        }

        template<typename T>
        auto read_register(const vm::register_name &src) {
            T value;
            memcpy(&value, &registers[src.index], sizeof(T));
            return value;
        }
    ...
    template<typename T>
    inline auto memory_address(const machine &m, const vm::register_name &dst) -> const T * {
        auto offset = m.registers[dst.index];
        if (offset + sizeof(T) > m.memory.size()) {
            throw std::out_of_range(
                    fmt::format("address {} exceeds allocated pool bounds {}", offset,
                                m.memory.size()));
        }
        return std::bit_cast<const T *>(m.memory.data() + offset);
    }

    template<typename T>
    inline auto memory_address(machine &m, const vm::register_name &dst) -> T * {
        auto offset = m.registers[dst.index];
        if (offset + sizeof(T) > m.memory.size()) {
            throw std::out_of_range(
                    fmt::format("address {} exceeds allocated pool bounds {}", offset,
        ...
        return std::bit_cast<T *>(m.memory.data() + offset);
    }
} // namespace tungsten